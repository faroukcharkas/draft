...
class SuggestionsModule:
    ...
    async def suggest(params: SuggestParams, writing_samples_index: PineconeIndex, openai: AsyncOpenAI) -> Suggestion:
        ...
        response: ChatCompletion = await openai.beta.chat.completions.parse(model="gpt-4o-mini", messages=[{"role": "system", "content": prompt}], response_format=Suggestion)
        
        # 5. Parse the response and create a Suggestion object
        suggestion: Suggestion = Suggestion.model_validate_json(response.choices[0].message.content)
